...
static int get_inbuilt_fw_path(struct lge_touch_data *ts, int panel_id)
{
	...
	if (ts->pdata->inbuilt_fw_name_list[panel_id] == NULL)
		...
		ts->pdata->inbuilt_fw_name = ts->pdata->inbuilt_fw_name_list[panel_id];
...
static int get_fw_ver_addr(struct lge_touch_data *ts, int panel_id)
{
	...
	if (!ts->pdata->fw_ver_addr_list[panel_id])
		...
		ts->pdata->fw_ver_addr = ts->pdata->fw_ver_addr_list[panel_id];
		...
		const char *buf, size_t count)
{
	...
	if (ts->pdata->panel_id) {
		TOUCH_DEBUG(DEBUG_BASE_INFO, "%s : panel_id = %d, Not using lpwg temporarily\n",
...
static struct touch_platform_data *get_dts_data(struct device *dev)
{
	...
	for (i = 0; i < rc; i++) {
		...
		TOUCH_INFO_MSG("fw_image [%d]: %s", i, p_data->inbuilt_fw_name_list[i]);
	}
	GET_PROPERTY_U32(np, "fw_pid_addr", p_data->fw_pid_addr);
	GET_PROPERTY_U32_ARRAY_2(np, "fw_ver_addr", p_data->fw_ver_addr_list, 2,
			NULL);
...
static int touch_resume(struct device *dev)
{
	...
	if (atomic_read(&ts->state.power_state) == POWER_ON && !mfts_enable) {
		TOUCH_INFO_MSG("%s : Power is already ON\n", __func__);
		return 0;
	}
...
#ifndef CONFIG_TOUCHSCREEN_SYNAPTICS_DISNEY
